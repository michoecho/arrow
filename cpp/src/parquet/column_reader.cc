...
seastar::future<std::shared_ptr<Page>> SerializedPageReader::NextPage() {
  ...
  return seastar::do_until(
    ...
    [=] () mutable {
      ...
      auto allowed_page_size = std::make_shared<uint32_t >(kDefaultPageHeaderSize);
      ...
      return seastar::repeat([=] () mutable {
        ...
        // We try to deserialize a larger buffer progressively
        // until a maximum allowed header limit
        return stream_->Peek(*allowed_page_size, buffer.get()).then([=] (int64_t peeked_bytes) mutable {
          if (buffer.get()->size() == 0) {
            *page_found = true;
            retval = std::shared_ptr<Page>(nullptr);
            return seastar::stop_iteration::yes;
          }

          // This gets used, then set by DeserializeThriftMsg
          *header_size = static_cast<uint32_t>(buffer.get()->size());
          try {
            if (crypto_ctx_.meta_decryptor != nullptr) {
              UpdateDecryption(crypto_ctx_.meta_decryptor, encryption::kDictionaryPageHeader,
                                data_page_header_aad_);
            }
            DeserializeThriftMsg(reinterpret_cast<const uint8_t*>(buffer.get()->data()),
        ...
        // Advance the stream offset
        return stream_->Advance(*header_size);
      }).then([=] () mutable {
        *compressed_len = current_page_header_.compressed_page_size;
        *uncompressed_len = current_page_header_.uncompressed_page_size;
        if (crypto_ctx_.data_decryptor != nullptr) {
          UpdateDecryption(crypto_ctx_.data_decryptor, encryption::kDictionaryPage,
                          data_page_aad_);
        }
