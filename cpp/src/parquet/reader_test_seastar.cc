...
public:
  void SetUp() {
    reader_ = seastarized::ParquetFileReader::Open(seastarized::ReadableRandomAccessFile::Open(alltypes_plain()).get0()).get0();
  }
...
TEST_F(TestAllTypesPlain, TestBatchRead) {
  ...
  std::shared_ptr<Int32Reader> col =
          std::dynamic_pointer_cast<Int32Reader>(group->Column(0).get0());
...
TEST_F(TestAllTypesPlain, TestFlatScannerInt32) {
  ...
  // column 0, id
  std::shared_ptr<Int32Scanner> scanner(new Int32Scanner(group->Column(0).get0()));
  int32_t val;
  bool is_null;
  for (int i = 0; i < 8; ++i) {
...
TEST_F(TestLocalFile, OpenWithMetadata) {
  // PARQUET-808
  std::stringstream ss;
  std::shared_ptr<FileMetaData> metadata = ReadMetaData(handle).get0();

  auto reader = ParquetFileReader::Open(handle, default_reader_properties(), metadata).get0();
...
//  printer.DebugPrint(ss, columns, true);
  ...
  auto fn =  seastarized::ReadableRandomAccessFile::Open(alltypes_plain()).get0();
  auto reader2 = seastarized::ParquetFileReader::Open(fn, default_reader_properties(), metadata).get0();
...
TEST(TestFileReaderAdHoc, NationDictTruncatedDataPage) {
  ...
  auto reader = seastarized::ParquetFileReader::Open(seastarized::ReadableRandomAccessFile::Open(nation_dict_truncated_data_page()).get0()).get0();
  std::stringstream ss;
  ...
  reader = seastarized::ParquetFileReader::Open(seastarized::ReadableRandomAccessFile::Open(nation_dict_truncated_data_page()).get0()).get0();
...
Column 1
  ...
  auto reader = ParquetFileReader::Open(ReadableRandomAccessFile::Open(data_file(file)).get0(), reader_props).get0();
  ParquetFilePrinter printer(reader.get());
...
TEST(TestJSONWithLocalFile, JSONOutput) {
  ...
  auto fs = seastarized::ReadableRandomAccessFile::Open(alltypes_plain()).get0();
  auto reader =
          ParquetFileReader::Open(fs, default_reader_properties()).get0();
  ParquetFilePrinter printer(reader.get());
...
TEST(TestFileReader, BufferedReads) {
  ...
  auto seastarized_file = std::make_shared<MemoryFutureOutputStream>(out_file);
  std::shared_ptr<ParquetFileWriter> file_writer =
          ParquetFileWriter::Open(seastarized_file, schema, writer_props).get0();

  RowGroupWriter *rg_writer = file_writer->AppendRowGroup().get0();
  ...
  for (int col_index = 0; col_index < num_columns; ++col_index) {
    DoubleWriter *writer = static_cast<DoubleWriter *>(rg_writer->NextColumn().get0());
