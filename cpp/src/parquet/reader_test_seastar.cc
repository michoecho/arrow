...
public:
  void SetUp() { reader_ = ParquetFileReader::OpenFile(alltypes_plain()).get0(); }
...
//  printer.DebugPrint(ss, columns, true);
  ...
  auto reader2 = ParquetFileReader::OpenFile(alltypes_plain(), false,
                                             default_reader_properties(), metadata).get0();
...
TEST(TestFileReaderAdHoc, NationDictTruncatedDataPage) {
  ...
  auto reader = ParquetFileReader::OpenFile(nation_dict_truncated_data_page(), false).get0();
  std::stringstream ss;
  ...
  reader = ParquetFileReader::OpenFile(nation_dict_truncated_data_page(), true).get0();
...
Column 1
  ...
  auto reader = ParquetFileReader::OpenFile(data_file(file), false, reader_props).get0();
  ParquetFilePrinter printer(reader.get());
...
TEST(TestJSONWithLocalFile, JSONOutput) {
  ...
  auto reader =
          ParquetFileReader::OpenFile(alltypes_plain(), false, default_reader_properties()).get0();
  ParquetFilePrinter printer(reader.get());
